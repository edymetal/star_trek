...
P0.1, P0.2 e P0.3 foram aprovados. A prioridade UI-1 passou pelo segundo ciclo e a fatia UI-GFX foi validada graficamente pelo coordenador, sem nova revisão do Product Architect por decisão explícita do usuário. A apresentação UI pura passa em 20/20 casos Chrome/Edge e o adaptador VFX em 10/10 unitários, mas a repetição final do fluxo gráfico-combate falhou em 0/2 porque o marcador deixou de estar visível antes do alinhamento. O ciclo 2 de correções do P0.4 continua bloqueado, assim como a matriz canônica completa. Não remova/ignore esse teste e não trate UI-GFX como conclusão do overhaul visual/benchmark P0.5 nem inicie conteúdo P1 antes dos gates correspondentes. A direção visual e seus limites estão em `DECISION-021` a `DECISION-023`.
